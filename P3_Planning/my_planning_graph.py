...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        # For each possible action see if its precondition nodes exist in the current 'S' level
        p_nodes = [PgNode_a(action) for action in self.all_actions] # All possible actions
        a_level = set([node for node in p_nodes if node.prenodes.issubset(self.s_levels[level])])
        # Add 'A' nodes to the children set of the 'S' nodes
        for node in self.s_levels[level]: 
            node.children |= a_level
        # Add 'S' nodes to the parents set fo the 'A' nodes
        for node in a_level: 
            node.parents |= self.s_levels[level]
    ...
    def add_literal_level(self, level):
        ...
        # Add all the effnodes from the previous 'A' level to the current 'S' level
        s_level = set([node for n in self.a_levels[level - 1] for node in n.effnodes]) 
        # Add 'S' nodes to the children of the 'A' nodes
        for node in self.a_levels[level - 1]:
            node.children |= s_level
        # Add 'A' nodes to the parents of the 'S' nodes
        for node in s_level: 
            node.parents |= self.a_levels[level - 1]
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # TODO test for Interference between nodes precond_neg
        # Check effects of 'node_a1' against preconditons of 'node_a2'
        if (set(node_a1.action.effect_add) & set(node_a2.action.precond_neg) or 
                set(node_a1.action.effect_rem) & set(node_a2.action.precond_pos)):
            return True
        # Check effects of 'node_a2' against preconditons of 'node_a1'
        if (set(node_a2.action.effect_add) & set(node_a1.action.precond_neg) or 
    ...
    def h_levelsum(self) -> int:
        ...
        for s_level in self.s_levels:
            symbols = set([node.symbol for node in s_level if node.is_pos])
            goals -= symbols
            level_sum += len(goals)
            if not goals: break
